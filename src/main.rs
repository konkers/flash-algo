...
unsafe fn lookup_func_v1(tag: u32) -> usize {
...
unsafe fn lookup_func_v2(tag: u32) -> usize {
...
fn find_func<T>(tag: [u8; 2]) -> Result<T, ErrorCode> {
    ...
    /// ROM as well as identifying different interfaces (referred here in as V1 and V2).
    const BOOTROM_MAGIC: *const [u8; 3] = 0x0000_0010 as _;

    /// Magic value for V1 ROMs.
    const V1_BOOTROM_MAGIC: &[u8; 3] = b"Mu\x01";

    /// Magic value for V2 ROMs.
    const V2_BOOTROM_MAGIC: &[u8; 3] = b"Mu\x02";

    unsafe {
        let result = match &*BOOTROM_MAGIC {
            V1_BOOTROM_MAGIC => lookup_func_v1(tag),
            V2_BOOTROM_MAGIC => lookup_func_v2(tag),
